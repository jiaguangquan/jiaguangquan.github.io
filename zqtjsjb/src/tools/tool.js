...
const tools = {
  ...
  // 统一社会信用代码
  isXydm: function(id) {
    var myreg = /^[^_IOZSVa-z\W]{2}\d{6}[^_IOZSVa-z\W]{10}$/g;
    if (!myreg.test(id)) {
      return false;
    } else {
      return true;
  ...
  // 英文数字校验 var pattern = /^0-9a-zA-z/
  isYwsz: function(cardStr) {
    // console.log(cardStr);
    var myreg = /^[0-9a-zA-z]/;
    if (!myreg.test(cardStr)) {
      return false;
  ...
  // 验证数字
  isParseFloat: function(cardStr) {
    // console.log(cardStr);
    var myreg = /^([\d]+(\.\d{1,3})?$)|^([1-9]+)$/;
    if (!myreg.test(cardStr)) {
      return false;
    } else {
      return true;
  ...
  // 邮政编码校验 var pattern = /^[0-9]{6}$/;
  isYzbm: function(cardStr) {
    // console.log(cardStr);
    var myreg = /^[0-9]{6}$/;
    if (!myreg.test(cardStr)) {
      return false;
